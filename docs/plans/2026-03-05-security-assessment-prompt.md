# Splintarr v1.3.0 — Comprehensive Security Assessment Prompt

> **Purpose:** This prompt is designed to be given to a Claude Code session with full tool access (Bash, Read, Grep, Glob, Playwright browser tools) to conduct both static code review and active penetration testing of the Splintarr application running in Docker.
...
You are conducting a **white-box security assessment** of Splintarr, a homelab application that automates backlog searching for Sonarr media management instances. The codebase is 100% AI-generated (Claude Code) and has never been reviewed by a human security professional.

**Your mission:** Find every exploitable vulnerability. Do not assume anything is safe because it "looks correct." AI-generated code has documented patterns of inconsistent validation, placeholder security, copy-paste gaps, and over-reliance on framework defaults. Verify everything empirically.
...
## Known Accepted Risks (Do Not Report as New)
...
## Pre-Identified Concerns (Investigate These First)
...
**All findings must be filed according to the repository's security policy (`SECURITY.md`).**

### Severity-Based Filing Protocol

| Severity | Filing Method | Rationale |
|----------|--------------|-----------|
| **Critical / High** | [GitHub Security Advisory](https://github.com/menottim/splintarr/security/advisories/new) | Private disclosure. Vulnerability details must not be public until a fix is released. |
...
gh api repos/menottim/splintarr/security-advisories --method GET 2>&1 | head -5

# If available, create an advisory (draft)
gh api repos/menottim/splintarr/security-advisories --method POST \
  -f summary="VULN-XX: [Title]" \
  -f description="[Full finding details including Description, PoC, Impact, Recommendation, CVSS, CWE]" \
  -f severity="high" \
  -f vulnerabilities='[{"package":{"ecosystem":"pip","name":"splintarr"},"vulnerable_version_range":"<= 1.3.0"}]'
```
...
3. **Affected version(s)** — `v1.3.0` (or `<= 1.3.0` if applicable to all versions)
...
# Then commit
git add docs/plans/2026-03-05-security-assessment-prompt.md
